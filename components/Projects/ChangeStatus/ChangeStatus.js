...
export default function ChangeStatus(props) {
  ...
  const createProjectCall = (statusId) => {
    setLoading(true);
    ChangeProjectStatus(props.project.id, statusId).then((res) => {
      if (res?.error) {
        message.error("Project Update failed!");
        setLoading(false);
      } else {
        message.success("Project Updated successfully!");
        props.setIsChangeProjectStatus(false);
      }
    });
  };
          ...
          <div className="flex flex-col items-center p-6 bg-gray-900 rounded-md">
            <div className="create-project-header flex flex-row justify-between w-full mb-6">
              ...
              <button
                ...
                onClick={() => props.setIsChangeProjectStatus(false)}
              >
                <RxCross1 size={24} />
              </button>
            </div>
            <div className="flex flex-col justify-center items-center">
              ...
              <h1 className="font-bold text-xl my-4">
                Change status of {props.project.project_name}
              </h1>
            </div>
