...
import Step1TeamCreation from "./Step1TeamCreation";
import Step2TeamCreation from "./Step2TeamCreation";
export default function CreateTeam(props) {
  const { data: session, status } = useSession();
  const [loading, setLoading] = useState(false);
  const [teamName, setTeamName] = useState(null);
  const [teamDetails, setTeamDetails] = useState(null);
  const [teamDomainSelected, setTeamDomainSelected] = useState(null);
  ...
  const createTeamCall = () => {
    ...
    createTeam(payload).then((res) => {
      ...
      if (res?.error) {
        ...
        props.setIsCreateTeam(false);
        props.setIsRefreshTeams(true);
      }
    });
  };
  ...
  useEffect(() => {
    ...
    getTeamDomains().then((res) => {
      console.log("Team Domains: ", res.data);
  ...
  return (
    <div className="create-team-container h-full w-full fixed overflow-hidden top-0 left-0 flex justify-center items-start">
      <motion.div
          ...
          <div className="flex flex-col items-center p-6 bg-gray-900 rounded-md">
            <div className="create-project-header flex flex-row justify-between w-full mb-6">
              ...
              <button
                ...
                onClick={() => props.setIsCreateTeam(false)}
              >
                <RxCross1 size={24} />
              </button>
            </div>
            <div className="flex flex-col justify-center items-center">
              ...
              <h1 className="font-bold text-xl my-4">Create a new Team</h1>
            </div>
